...
export async function GET(_req: NextRequest) {
  try {
    ...
    if (error) {
      console.warn('[/api/liked GET] DB error:', error.message)
    ...
    console.warn('[/api/liked GET] Unexpected error:', err)
    return NextResponse.json({ success: true, data: [] })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.warn('[POST /api/liked DB error]:', error.message)
      return NextResponse.json({ success: true })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.warn('[POST /api/liked error]:', err)
    return NextResponse.json({ success: true })
  }
}
